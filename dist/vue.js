...
(function (global, factory) {
  ...
  function nextTick(cb, ctx) {
    ...
    callbacks.push(function () {
      if (cb) {
        // 用 try catch 包装回调函数，便于错误捕获，因为这个函数是可以被用户调用的，所以保证稳定性，肯定是需要做错误提前捕获的准备
        try {
          cb.call(ctx);
        } catch (e) {
  ...
  Watcher.prototype.get = function get () {
    ...
    var vm = this.vm;
    try {
      // 执行回调函数，比如 updateComponent，进入 patch 阶段，或者用户watch传递的key，可能是读取this.key的函数，进行依赖收集
  ...
  function proxy(target, sourceKey, key) {
    ...
    // 实现拦截，相当于访问this.propKey 也就是会访问this.sourceKey.propKey
  ...
  function initState(vm) {
     ...
     *   2、代理 data 对象上的属性到 vm 实例，支持this.key的方式访问data
  ...
  function initData(vm) {
    ...
    while (i--) {
      ...
      if (props && hasOwn(props, key)) {
        ...
        // 代理 data 对象上的属性到 vm 实例，支持this.key的方式直接访问data
        proxy(vm, "_data", key);
      }
    }
  ...
  function initComputed(vm, computed) {
    ...
    for (var key in computed) {
      ...
      if (!isSSR) {
        ...
        // 为 computed 属性创建 watcher 实例，所以computed其实就是通过watch实现的
      ...
      if (!(key in vm)) {
        // 代理 computed 对象中的属性到 vm 实例 这样就可以使用 vm.computedKey 访问计算属性了
  ...
  function createComputedGetter(key) {
    ...
    return function computedGetter() {
      ...
      if (watcher) {
        ...
        if (watcher.dirty) {
          // 内部执行了computed对应的key函数，将返回值放到了watcher.value上
          watcher.evaluate();
        }
        if (Dep.target) {
          watcher.depend();
